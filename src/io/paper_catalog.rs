//! Paper catalogue with AutoCAD-compatible media names.
//!
//! AutoCAD stores a layout's paper as the plotter driver's *canonical media
...
//! saved here has to carry those exact names so AutoCAD finds the media in its
//! `DWG To PDF.pc3` list instead of silently falling back to a default sheet,
//! and a name that arrives from AutoCAD (including a custom size this catalogue
...
impl PaperUnits {
    ...
    /// The unit token AutoCAD writes inside the media name.
...
/// AutoCAD offers three printable-area variants of most sheets: the standard
...
impl PaperVariant {
    /// The infix AutoCAD places between the series and the sheet letter.
...
pub struct PaperSize {
    /// AutoCAD canonical media name, e.g. `ISO_A4_(210.00_x_297.00_MM)`. This is
...
impl PaperSize {
    ...
    /// A user-defined sheet, named the way AutoCAD names custom media created
    ...
    /// Rebuild a sheet from any media name that follows AutoCAD's
...
/// `<prefix>_(<w>_x_<h>_<units>)` exactly as AutoCAD spells it: two decimals,
/// portrait order.
...
/// Compact source row: series prefix as AutoCAD writes it, sheet letter,
/// portrait width/height, units, and whether AutoCAD ships full-bleed /
...
/// The sheets AutoCAD's `DWG To PDF.pc3` offers, in its display order, plus
...
/// an unknown but well-formed AutoCAD name, which is reconstructed from its
...
mod tests {
    ...
    #[test]
    fn catalogue_names_follow_autocad_spelling() {
    ...
    #[test]
    fn unknown_autocad_names_are_reconstructed_and_preserved() {
        // A custom size created in AutoCAD's plot dialog.
        ...
        assert_eq!(custom.portrait_mm(), (420.0, 297.0));
