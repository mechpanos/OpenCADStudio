...
//! can print.
//!
//! AutoCAD stores the device in `PlotSettings::printer_name` as the plotter
...
//! drawing saved here has to use the same spelling so AutoCAD attaches the
//! layout to its own PDF driver instead of reporting a missing device; and a
//! device name that arrives from AutoCAD (an HP plotter's `.pc3`, `DWF6
...
//! driver's unprintable margins, which AutoCAD writes into the layout and
...
pub enum PlotDevice {
    /// No plotter assigned (AutoCAD `None`). We plot to the system default
    /// printer in that case, which is what the plot dialog offers.
    None,
    /// PDF file output, AutoCAD's `DWG To PDF.pc3`.
    Pdf,
...
impl PlotDevice {
    ...
    /// stored spelling should be preserved on save: an AutoCAD plotter
    ...
    /// to that plotter in AutoCAD; our own legacy label and an empty name are
    ...
    pub fn from_stored_name(name: &str) -> (Self, bool) {
        ...
        if lower.ends_with(".pc3") {
            // Any AutoCAD plotter configuration: PDF ones are ours to drive,
            // the rest plot to PDF here but keep their name for AutoCAD.
...
mod tests {
    ...
    fn stored_names_resolve_and_say_whether_to_keep_them() {
        ...
        assert_eq!(
            PlotDevice::from_stored_name("AutoCAD PDF (High Quality Print).pc3"),
            (PlotDevice::Pdf, true)
        );
